...
Custom filament presets, organized by slicer, then filament vendor, then Bambu Lab printer model. Currently covers **Bambu Studio** `.bbsflmt` bundles for **TINMORRY** and **eSUN** filaments; the repository layout is structured to add further slicers (e.g. OrcaSlicer) and vendors (e.g. ELEGOO) later without another reorganization.

## ⚠️ Caution — read before downloading or using

- Profiles marked **Derived** in the table below are **not original vendor exports**. They were generated by merging the vendor's own smaller per-printer delta data onto another bundle's machine parameters (see `CLAUDE.md` / `REFERENCES.md`) and have **not** been print-validated by TINMORRY or eSUN.
...
Filament profile data lives under `profiles/`, the toolchain that generates it under `src/`. Inside `profiles/`, each top-level folder is a slicer/export format; each folder under that is a filament vendor; each folder under that corresponds to a printer model (as reported in each profile's `compatible_printers` field). `X2D` additionally nests profiles under a `0.4mm` nozzle folder. `BambuStudio/TINMORRY/` and `BambuStudio/eSUN/` have profiles today — further slicer folders (OrcaSlicer) and vendor folders (ELEGOO) will appear the same way once sourced.
...
3. The filament preset will appear under the matching printer in the filament dropdown, under the `TINMORRY` or `eSUN` vendor (matching whichever bundle you imported).
