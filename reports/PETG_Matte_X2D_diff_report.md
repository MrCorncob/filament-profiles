...
**Recommendation:** if this matters for real prints on a Bowden-extruder X2D, this is exactly the kind of correction the repo's new custom-override mechanism (`src/custom_overrides/TINMORRY/X2D/0.4mm/TINMORRY PETG Matte.json`) was built for — it can force the correct per-slot values without changing the generic merge convention used by every other bundle.
